Node given in IsaacSim-ros_workspaces for controling the panda arm. Node simply publishes a time varying joint space position command to the joint_command topic. The Isaac Sim articulation controller node in the action graph handles the actual joint torque control. 
